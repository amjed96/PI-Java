...
public class AffecterController implements Initializable{
    ...
    public static DonService donService = new DonService();
  ...
  @FXML
	...
	private void displaySujetDetails(Node node, User user) throws SQLException {
		...
		item_last.setText(user.getLast_Name());
                JFXComboBox<String> item_don = (JFXComboBox) node.lookup(".item_don");
		item_don.getItems().addAll(donService.readAllByLevel1(choose_level.getValue()));
               // System.out.println(item_don.getSelectionModel().getSelectedItem());
	...
	private EventHandler<MouseEvent> OpenEventHandler(Node node,User user, int index) {
		return event -> {
                   JFXComboBox<String> item_don = (JFXComboBox) node.lookup(".item_don");
                    System.out.println(item_don.getValue());
                        user.setClasse(item_don.getValue());
                    try {
